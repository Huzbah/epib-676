...
```{r}
expected_cost_diet <- (c_diet + c_stent)*p_stent_diet + c_diet*(1-p_stent_diet)
...
chance_node__OTS_then_ST <- ChanceNode$new("Second-line therapy sucess?")
chance_node__ST_then_OTS <- ChanceNode$new("Second-line therapy sucess?")
chance_node__ST_then_ST <- ChanceNode$new("Second-line therapy sucess?")

#Create leaf nodes
# (9 total. "1st line success", "2nd line success", and "IR needed" for each scenario)

leaf_node_OTS_1success <- LeafNode$new("First-line therapy successful", utility = params$q_single_EGD)
leaf_node_OTS_2success <- LeafNode$new("Second-line therapy successful", utility = params$q_double_EGD)
leaf_node_OTS_ir_needed <- LeafNode$new("IR needed",  utility = params$q_IR)

leaf_node_ST_first_1success <- LeafNode$new("First-line therapy successful",  utility = params$q_single_EGD)
leaf_node_ST_first_2success <- LeafNode$new("Second-line therapy successful",  utility = params$q_double_EGD)
leaf_node_ST_first_ir_needed <- LeafNode$new("IR needed",  utility = params$q_IR)

leaf_node_ST_only_1success <- LeafNode$new("First-line therapy successful",  utility = params$q_single_EGD)
leaf_node_ST_only_2success <- LeafNode$new("Second-line therapy successful",  utility = params$q_double_EGD)
leaf_node_ST_pnly_ir_needed <- LeafNode$new("IR needed",  utility = params$q_IR)
...
reaction_OTS_success <- Reaction$new(
  ...
  p = 1-params$p_OTSc_rebleed, cost = params$c_hosp_noCC, label = "Did not need second line therapy")

reaction_ST_first_success <- Reaction$new(
  chance_node_ST_first, leaf_node_ST_first_1success, 
  p = 1-params$p_ST_rebleed, cost = params$c_hosp_noCC, label = "Did not need second line therapy")
...
reaction_OTS_failure <- Reaction$new(
  chance_node_OTS_first, chance_node__OTS_then_ST, 
  p = params$p_OTSc_rebleed, cost = params$c_STclip, label = "Needed second line therapy")

reaction_ST_first_failure <- Reaction$new(
  chance_node_ST_first, chance_node__ST_then_OTS, 
  p = params$p_ST_rebleed, cost = params$c_OTSclip, label = "Needed second line therapy")

reaction_ST_only_failure <- Reaction$new(
  chance_node_ST_only, chance_node__ST_then_ST, 
  p = params$p_ST_rebleed, cost = params$c_STclip, label = "Needed second line therapy")
...
reaction_ST_then_OTS_success <- Reaction$new(
  chance_node_ST_then_OTS, leaf_node_ST_first_1success, 
  p = 1-params$p_ST_rebleed, cost = params$c_hosp_noCC, label = "Did not need second line therapy")
...
-   About how much time did you spend on the assignment? **2 hours**
